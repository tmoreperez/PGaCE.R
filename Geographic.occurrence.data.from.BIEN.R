#This script will extract occurrence data from the BIEN database, retrieves climate
...
sclimout=foreach(i=1:length(splist), .combine=rbind, .packages = c("BIEN", "sp", "raster")) %dopar% {
  #Get geographic occurence data from the BIEN database. See the documentations 
  #"?BIEN_occurrence_species" for more information about this function
  biendistdata1=BIEN::BIEN_occurrence_species(splist[i], cultivated = FALSE, only.new.world = T, natives.only = F, political.boundaries = F, collection.info = T)
  
  if( length(biendistdata1)>1){ #Make sure that there is occurrence data for a species 
    biendistdata1$unscrubbed.name=rep(splist[i], length(biendistdata1[,1])) #Paste the original name you entered into the extracted data.frame (BIEN will return correcteed/"scrubbed" names)
    if( length(which( complete.cases(biendistdata1[,1:3])==T) )>=15 ){ #Get complete cases and only use if there are 15 or more occurrences
      biendistdata2=biendistdata1[complete.cases(biendistdata1[,1:3]),] #drop columns that aren't needed
      
      #Extract the climate data for each coordinate
      coords = data.frame(longitude=biendistdata2$longitude, latitude=biendistdata2$latitude)
      ...
      climdat2 = cbind.data.frame(raster::coordinates(points),values, 
                                  names= rep(splist[i], length(biendistdata2[,1])),
                                   scrubbed_species_binomial=biendistdata2$scrubbed_species_binomial,
                                   date_collected= biendistdata2$date_collected)
      ...
      sclimout[[i]]=data.frame(scrubbed_species_binomial=unique(cleaned.fwdist.clim$scrubbed_species_binomial),
